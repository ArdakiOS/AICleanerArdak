...
struct DuplicatePhotosFullView: View {
    let columns = [
    ...
    var body: some View {
        ZStack{
            ...
            VStack(spacing: 20){
                ...
                HStack{
                    Text("\(selectedImgs.count) photos")
                    ...
                    Button{
                        selectedImgs = vm.duplicatePhotos
                ...
                ScrollView(.vertical) {
                    LazyVGrid(columns: columns, spacing: 20) {
                        ForEach(vm.duplicatePhotos, id: \.self){album in
                ...
                ZStack{
                    ...
                    VStack(spacing: 15){
                        ...
                        HStack{
                            ...
                            Button{
                                vm.deleteDuplicates(assets: selectedImgs, from: "photo")
                                showDeletePrompt.toggle()
                            } label: {
                                Text("Delete")
                                    .font(.custom(FontExt.semiBold.rawValue, size: 15))
    ...
    private func calculateTotalSize() async {
        for i in selectedImgs {
            totalSize += i.duplicates.fileSize()
        }
    }
}

#Preview {
    DuplicatePhotosFullView(vm: PhotoGalleryViewModel())
}
